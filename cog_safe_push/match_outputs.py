...
async def audios_match(
    url1: str,
    url2: str,
    is_deterministic: bool,  # noqa: ARG001 TODO: use is_deterministic
) -> tuple[bool, str]:
    # TODO: is_deterministic branch - compare audio waveforms or similar
    with download(url1) as tmp1, download(url2) as tmp2:
        fuzzy_match = await ai.boolean(
            "Have these two audio files been generated by the same inputs to a generative AI model? Listen to both audio files and determine if they have the same or very similar content.",
            files=[tmp1, tmp2],
        )
    if fuzzy_match:
        return True, ""
